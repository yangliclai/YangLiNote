...
def detail(request, blog_id):
    # objects.get() will return only 1 result as default setting.
...
def search(request):
    # if no "keyword" in form but you enforce to press "submit" btn. The following code will cause you to early return
    keyword = request.GET.get('keyword', None)
    if not keyword:
        error_msg = "请输入关键字"
        # use this my hands-on return<redirect> , we will get a fixed return to blog homepage
        return HttpResponseRedirect('/blog/')
        # use the below original return<request,para1,para2> , we will get a queryset return with keyword= None
        # return render(request, 'blog/index.html', locals())
...
def login(request):
    ...
    access_token_url = 'https://api.weibo.com/oauth2/access_token?client_id=%s&client_secret=%s&grant_type=authorization_code&redirect_uri=http://www.yanglinote.com/login&code=%s'\
